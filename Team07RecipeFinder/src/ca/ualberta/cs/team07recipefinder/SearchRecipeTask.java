...
public class SearchRecipeTask extends AsyncTask<String, List<String>, Void> {
	/**
	 * type that is overridden so as to call a method inside the calling
	 * activity to pass results back.
	 */
	DataDownloadListener dataListener;
	/**
	 * list of recipes that is returned from the call to the elastic search
	 * server.
	 */
	...
	/**
	 * If only seearching for a single recipe it will search by UUID.
	 */
	UUID recipeId;
	 ...
	 * 
	 * @param recipeTitleKeyword
	 *            keyword to search for in recipe titles
	 * @param ingredients
	 *            list of ingredients to search for in recipe
	 */
	...
	/**
	 * Constructor for when only searching for a single recipe
	 * 
	 * @param recipeId
	 ...
	 *            object to trigger when async task is done and pass data back
	 *            out
	 */
